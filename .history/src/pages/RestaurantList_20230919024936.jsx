...
const RestaurantList = () => {
  ...
  const changedCategoriesHandler = (newCategories) => {
    setCategories(newCategories);
  };
  let rating;
  useEffect(() => {
    ctx.getRestaurants([
      initialCategory === "Show All" ? "Show All" : initialCategory,
    ]);
  }, []);
  if (ctx.restaurants) {
    // const fullScore=ctx.restaurants.ratings(rating=> )
    console.log(ctx.restaurants.ratings);
  }
  return (
    <>
        ...
        {open && (
          ...
          >
            <div className="absolute top-[15%] sm:left-[10%] left-[3%] lg:left-[25%] md:w-[600px] w-3/4 m-10 border-2 border-neutral-400 bg-secondary">
              <CategoryForm
                onChange={changedCategoriesHandler}
