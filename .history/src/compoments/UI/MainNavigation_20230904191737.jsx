      ...
      <div className="flex justify-between ">
        ...
        <nav className="w-min mt-3 flex">
         ...
         font-sans"
          ...
          {ctx.isLoggedIn && (
            <NavLink
              className="flex w-14 md:w-28 justify-around rounded-2xl text-center border-neutral-500 border-solid border-2 p-2.5 sm:w-36   "
              to="account"
            >
              {<UserIcon />} <span className="hidden md:inline">Account</span>
            </NavLink>
          )}
          {ctx.isLoggedIn && <button type="button">Log Out</button>}
        </nav>
      </div>
    </header>
  );
